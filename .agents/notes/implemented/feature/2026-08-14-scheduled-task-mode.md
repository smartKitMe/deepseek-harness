...
`@deepseek-ai/dsh-scheduled-task` is a durable, cross-session scheduler. Task definitions live in a `storage-domain` table (keyed by a branded `ScheduledTaskId`), so they survive restarts and belong to no Session. One record carries a trimmed `name` and `prompt`, a discriminated `schedule` (`cron` with expression + IANA zone, or `interval` with a minimum `everySeconds`), a `model` route, a `permission` preset name, a `conversation` mode (`new`, `task-session`, or `session` with a `sessionId`), an optional absolute `cwd` (absent runs in the host process working directory), `enabled`, and run state (`lastRunAt`, `lastRunSessionId`, `lastRunError`).
